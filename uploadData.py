...
def process_tesla(cable, fname):
    ...
    lines = fname.getvalue().decode("utf-8").splitlines()
    try:
    ...
    for line in enumerate(header_lines):
    ...
    if(is_1s_leakage(test_name) or is_leakage(test_name)):
        ...
        if(is_1s_leakage(test_name) or is_leakage(test_name)):
            ...
            if(is_leakage(test_name)):
                ...
                filtered_path = output_root / {cable.length} / {cable.serial_number}
